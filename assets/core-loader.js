...
(async()=>{
  ...
  const loadScript=src=>new Promise((resolve,reject)=>{
    ...
    const s=document.createElement('script');s.src=src.startsWith('assets/')?`${src}?v=past-papers-1`:src;s.async=true;s.onload=()=>{s.dataset.ffLoaded='1';resolve()};s.onerror=()=>reject(new Error(`Could not load ${src}`));document.body.appendChild(s);
  });
  const loadStyle=href=>new Promise((resolve,reject)=>{
    ...
    const l=document.createElement('link');l.rel='stylesheet';l.href=`${href}?v=past-papers-1`;l.onload=resolve;l.onerror=()=>reject(new Error(`Could not load ${href}`));document.head.appendChild(l);
  });
