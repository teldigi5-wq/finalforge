...
(()=>{
  ...
  style.textContent=`
    ...
    html.ff-auth-restoring body.auth-pending #authGate::after{content:'Restoring your session…'!important;position:fixed!important;z-index:80!important;left:50%!important;top:50%!important;right:auto!important;bottom:auto!important;width:auto!important;height:auto!important;min-width:13rem!important;transform:translate(-50%,-50%)!important;filter:none!important;opacity:1!important;padding:.78rem 1rem .78rem 2.55rem!important;border:1px solid rgba(120,158,213,.22)!important;border-radius:999px!important;background:linear-gradient(135deg,rgba(13,27,47,.96),rgba(8,18,33,.98))!important;color:#dce9f8!important;box-shadow:0 18px 50px rgba(0,0,0,.28)!important;font:750 .82rem/1.2 Inter,ui-sans-serif,system-ui,sans-serif!important;letter-spacing:.01em!important;text-align:center!important;pointer-events:none!important}
    html.ff-auth-restoring body.auth-pending #authGate::before{content:''!important}
    html[data-theme='light'].ff-auth-restoring body.auth-pending #authGate::after{border-color:#d0deeb!important;background:rgba(255,255,255,.96)!important;color:#27415f!important;box-shadow:0 18px 45px rgba(43,72,107,.14)!important}
    @media(prefers-reduced-motion:no-preference){html.ff-auth-restoring body.auth-pending #authGate::after{animation:ffSessionRestorePulse 1.4s ease-in-out infinite alternate!important}@keyframes ffSessionRestorePulse{from{opacity:.72}to{opacity:1}}}
  `;
...
(async()=>{
  const fail=(msg)=>{
    ...
    const gate=document.getElementById('authGate');if(gate){const n=document.getElementById('authConfigNote');if(n){n.hidden=false;n.innerHTML=`<b>FinalForge could not start.</b><br>${msg}`}}
  };
  ...
  try{
    ...
    const stylesReady=Promise.all([
      loadStyle('assets/ui-responsive-v2.css'),loadStyle('assets/auth-experience-v4.css'),loadStyle('assets/dashboard-modern-v3.css'),loadStyle('assets/mobile-modern-v4.css'),loadStyle('assets/product-ui-v4.css'),loadStyle('assets/product-ui-v5.css'),loadStyle('assets/tailwind.generated.css'),loadStyle('assets/auth-premium-v5.css'),loadStyle('assets/reference-refresh.css'),loadStyle('assets/experience-v9.css'),loadStyle('assets/auth-world-v1.css'),loadStyle('assets/theme-coherence-v1.css'),loadStyle('assets/responsive-hardening-v2.css'),loadStyle('assets/auth-neon-rounded-v1.css'),loadStyle('assets/past-papers-v1.css'),loadStyle('assets/student-experience-v2.css'),loadStyle('assets/visual-system-v2.css'),loadStyle('assets/mobile-auth-v5.css')
    ]);

    const firebaseSdkReady=(async()=>{
      await Promise.all([loadScript('assets/firebase-config.js'),loadScript('https://www.gstatic.com/firebasejs/10.14.1/firebase-app-compat.js')]);
      await Promise.all([loadScript('https://www.gstatic.com/firebasejs/10.14.1/firebase-auth-compat.js'),loadScript('https://www.gstatic.com/firebasejs/10.14.1/firebase-firestore-compat.js')]);
    })();

    const paths=['00','01','02','03','04','05a','05b','05c','05d','06'].map(x=>`assets/core/chunk-${x}.txt`);
    const parts=await Promise.all(paths.map(async p=>{const r=await fetch(p,{cache:'force-cache'});if(!r.ok)throw new Error(`Core bundle chunk failed (${p}, ${r.status}).`);return r.text()}));
    await stylesReady;
    ...
    const b=JSON.parse(await new Response(stream).text());
    ...
    (0,eval)(b['data.js']);if(!window.FINALFORGE_DATA&&window.EXAMHUB_DATA)window.FINALFORGE_DATA=window.EXAMHUB_DATA;(0,eval)(b['practice-data.js']);
